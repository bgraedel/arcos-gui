...
class ARCOS:
    """
    class to calculate collective events with arcos algroithm. requires r installation and the r packages arcos and data.table
    column argument requires a dictionarry of columns: "frame","x_coordinates","y_coordinates","track_id", "measurment", 'field_of_view_id'
    e.g. 
    dicCols = {
        'frame': frame, 
    ...
    def __init__(self, dataframe, columns : dict):
        ...
        if not all(key in self.columns for key in ['frame','x_coordinates','y_coordinates','track_id', 'measurment', 'field_of_view_id']):
            print("not all required keys are in the dict, requires: \n 'frame','x_coordinates','y_coordinates','track_id', 'measurment', 'field_of_view_id'")
    ...
    def create_arcosTS(self, interval = 1, inter_type = 'fixed', return_dataframe = False):
        """
        method to create an arcos object. returns pandas dataframe if return_dataframe = True
        """
        with ro.conversion.localconverter(ro.default_converter + pandas2ri.converter):
            self.datatable = ro.conversion.py2rpy(self.dataframe)
        self.datatable = datatable.as_data_table(self.datatable)

        lista = ro.vectors.ListVector({'Frame': self.columns['frame'], 'IDobj': self.columns['track_id']})
        pos_variables_r = ro.StrVector([self.columns['x_coordinates'],self.columns['y_coordinates']])
  
        self.datatable = arcos.arcosTS(self.datatable, 
        ...
        if return_dataframe: return self.convert_to_pd(self.datatable)
    ...
    def clip_measurements(self, clip_low = 0.001, clip_high = 0.999, return_dataframe = False):
        """
    ...
    def bin_measurements(self, biasmethod = 'runmed', smooth_k = 1, bias_k = 25, peak_thr = 0.2, bin_thr = 0.1, polyDeg = 1, return_dataframe = False):
        """
        binarizes measurements
        """
        self.datatable = arcos.binMeas(self.datatable,
                            biasMet = biasmethod, 
                            smoothK = smooth_k,
                            biasK = bias_k,
                            polyDeg = polyDeg,
    ...
    def track_events(self, neighbourhood_size = 40, min_clustersize = 5, return_dataframe = False):
        """
        method to track collective events of an arcos object. can return pandas dataframe
        """
        self.ts = self.datatable
        self.ts = arcos.trackColl(filter(self.ts, self.ts.rx2('meas.bin').ro > 0), eps = neighbourhood_size, minClSz = min_clustersize)
        if return_dataframe: return self.convert_to_pd(self.ts)

    def filter_tracked_events(self, min_duration = 3, total_event_size = 30, as_pd_dataframe = False):
        """
        ...
        self.tcollsel = arcos.selColl(self.ts, 
                        colldur = min_max_duration, 
                        colltotsz = tot_event_size)
        if as_pd_dataframe: return self.convert_to_pd(self.tcollsel)
        else: return self.tcollsel
...
class process_input:
    def __init__(self, csv_file = None, df = None, columns: dict = None):
        if df is not None:
            self.df = df
        else:
    ...
    def filter_position(self, fov_to_select = None, return_dataframe = False):
        if fov_to_select is not None:
            self.df = self.df.loc[self.df.loc[:,self.columns['field_of_view_id']] == fov_to_select]
        if return_dataframe: return self.df

        # filter_tracklenght works only if previously filtered by pos or if track_ids dont overlapp between fov
    def filter_tracklength(self, min, max, return_dataframe = False):
        ...
        self.df = self.df.loc[self.df.loc[:,self.columns['track_id']].isin(track_length_filtered_names)] 
        if return_dataframe: return self.df
