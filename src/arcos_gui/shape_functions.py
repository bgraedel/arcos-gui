...
def make_shapes(df, col_id='index', col_x='axis-2', col_y='axis-1', col_t='axis-0', col_colors='color', col_text=None):
    """
    ...
    # Drop irrelevant columns and check that the columns are ordered according to napari's format
    ...
    l_shapes = [dff.drop(columns=[col_id, col_colors, col_text], errors='ignore') for dff in l_shapes]
    out['data'] = [dff.to_numpy() for dff in l_shapes]
    return out

def make_timestamp(viewer, start_time=0, step_time=1, prefix='T = ', suffix=' frame', position = 'upper_left',size=12, x_shift=0, y_shift=0):
    """
    Create a timestamp displayed in the viewer.
    This is done by creating a dummy shape layer and annotating it with the current time.
    """
    anchors = ['upper_right', 'upper_left', 'lower_right', 'lower_left', 'center']
    if position not in anchors:
        raise ValueError('"position" must be one of: {}'.format(anchors))
    ...
    out['properties']['label'] = ['{}{}{}'.format(prefix, str(i), suffix) for i in timestamp]
    out['data'] = corners
...
def get_verticesHull(df, col_x, col_y):
    """From a set of point coordinates (XY), return the points coordinates which form the vertices of the convex hull.

    Args:
        df (pd.DataFrame): A dataframe with at least 2 columns containing the XY coordinates of a set of points.
    """
...
def format_verticesHull(df, col_time, col_x, col_y, col_collid):
    ...
    Args:
        df (pd.DataFrame): A DataFrame with the coordinates of the vertices of collective events.
    """
